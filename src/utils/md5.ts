...
interface BinArray extends Array<number> { }
...
function str2binl(str: string): BinArray {
    const bin: BinArray = [];
    const mask = (1 << chrsz) - 1;

    for (let i = 0; i < str.length * chrsz; i += chrsz) {
        const index = i >> 5;
        bin[index] = bin[index] || 0;
        bin[index] |= (str.charCodeAt(i / chrsz) & mask) << (i % 32);
    }
    return bin;
}

function binl2hex(binarray: BinArray): string {
    const hex_tab = hexcase ? "0123456789ABCDEF" : "0123456789abcdef";
    let str = "";

    for (let i = 0; i < binarray.length * 4; i++) {
        const byte1 = (binarray[i >> 2] >> ((i % 4) * 8 + 4)) & 0xF;
        const byte2 = (binarray[i >> 2] >> ((i % 4) * 8)) & 0xF;
        str += hex_tab.charAt(byte1) + hex_tab.charAt(byte2);
    }
    return str;
}

function core_md5(x: BinArray, len: number): BinArray {
    /* append padding */
    x[len >> 5] |= 0x80 << ((len) % 32);
    x[(((len + 64) >>> 9) << 4) + 14] = len;
...
function core_hmac_md5(key: string, data: string): BinArray {
    let bkey = str2binl(key);
    if (bkey.length > 16) bkey = core_md5(bkey, key.length * chrsz);

    const ipad: BinArray = new Array(16).fill(0x36363636);
    const opad: BinArray = new Array(16).fill(0x5C5C5C5C);
